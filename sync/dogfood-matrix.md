...
### 4. Synthetic Project-Local Fact Isolation
...
| Profile | `aiplus status` | _PENDING_ | `STATUS=PASS`, installed=yes |
...
- `aiplus profile install` copies only `AGENTS.profile.md` and `profile.toml`
  ...
  `preferences/`, `sync/` remain in the source repository and
